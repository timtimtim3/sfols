...
class SF(RLAlgorithm):
    ...
    def set_augmented_psi_table(self, n_fsa_states, feat_dim, indicator_edge_has_proposition, states):
        self.augmented_psi_table = dict()

        for uidx in range(n_fsa_states):
            self.augmented_psi_table[uidx] = dict()
            for state in states:
                augmented_psis = np.zeros((self.action_dim, n_fsa_states * feat_dim))
